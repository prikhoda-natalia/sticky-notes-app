...
export function useNoteCardInteractions({
  ...
  const handleDragging = useCallback(
    (e: PointerEvent) => {
      ...
      if (noteEvent.mode === 'move') {
        ...
        nextNoteGeometry = clampNoteOnBoardPosition(nextNoteGeometry, noteEvent.boardOnClient);
      } else {
        nextNoteGeometry = {
          ...noteEvent.noteOnBoard,
        ...
        nextNoteGeometry = clampNoteOnBoardSize(nextNoteGeometry, noteEvent.boardOnClient);
      }
  ...
  const handleDraggingEnd = useCallback(
    (e: PointerEvent) => {
      ...
      (e.currentTarget as HTMLElement).releasePointerCapture(noteEvent.pointer.id);
